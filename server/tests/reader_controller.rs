...
mod tests {
    ...
    use beer_core::{dtos::{BeerRequest, BeerResponse}, entities::UserToken};
    ...
    async fn test_add_beer() {
        ...
        assert!(req.status().is_success(), "Failed to request beer for existing user");
        let resp: BeerResponse = test::read_body_json(req).await;
        assert!(resp.valid, "User was valid to register a new beer");

        // Clean up
        let client: Client = pool.get().await.map_err(MyError::PoolError).unwrap();
        let user = beer_core::user_service::get_user(&client, &UserToken::parse_from_string("1A4215B1".to_string()).unwrap()).await.unwrap();
